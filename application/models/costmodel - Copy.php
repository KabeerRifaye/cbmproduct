...
{         
	...
	function createcostdetails()
    {
		...
		$lastData = $this->getLastCostDetails($userDetails['id']);
	...
	function getLastCostDetails($userid){

		 $query ="select * from costdetails where id_user = ".$userid." order by id_costdetails DESC limit 1";
	...
	function getReport(){
		...
		if(isset($_POST['pDateTo']) && $_POST['pDateTo']!=''){
			$where_p .= 'cost.purchasedate >= '.date('Y-m-d',strtotime($_POST['pDateTo'])). ' and ';
			//$this->db->where('cost.purchasedate >=',date('Y-m-d',strtotime($_POST['pDateTo'])));
		}
		if(isset($_POST['pDateFrom']) && $_POST['pDateFrom']!=''){
			$where_p .= 'cost.purchasedate <= '.date('Y-m-d',strtotime($_POST['pDateFrom']));
			//$this->db->where('cost.purchasedate <=',date('Y-m-d',strtotime($_POST['pDateFrom'])));
		}
		if(isset($_POST['aDateTo']) && $_POST['aDateTo']!=''){
			$where_a .= 'cost.arrivaldate >= '.date('Y-m-d',strtotime($_POST['aDateTo'])). ' and ';
			//$this->db->or_where('cost.arrivaldate >=',date('Y-m-d',strtotime($_POST['aDateTo'])));
		}
		if(isset($_POST['aDateFrom']) && $_POST['aDateFrom']!=''){
			$where_a .= 'cost.arrivaldate <= '.date('Y-m-d',strtotime($_POST['aDateFrom']));
			///$this->db->or_where('cost.arrivaldate <=',date('Y-m-d',strtotime($_POST['aDateFrom'])));
		}
